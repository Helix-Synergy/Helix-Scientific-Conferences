    ...
    useEffect(() => {
        const fetchToken = async () => {
            ...
            try {
                ...
                const response = await fetch(`https://backend-code-6vqy.onrender.com/api/source/generate-token?sourceId=${sourceId}&conferenceType=${conferenceType}`);
...
const Webinars = () => {
    ...
    useEffect(() => {
        const fetchGenericToken = async () => {
            ...
            try {
                const response = await fetch(`https://backend-code-6vqy.onrender.com/api/source/generate-token?sourceId=${genericSourceId}&conferenceType=${genericConferenceType}`);
                if (!response.ok) {
                    const errorData = await response.json();
