...
//         // Example token - you might get it from localStorage or API
...
//           { orderDetails }, // sending orderDetails in body
//           {
//             headers: {
//               "x-access-token": token, // sending token in headers
//             },
//           }
//         );
...
//         // 2️⃣ Verify token (THIS returns prices also)
...
//         // 3️⃣ Extract ONLY prices
//         setPrices(verifyRes.data.pricingPlans || []);
...
//           {prices.map((plan) => (
//             <li key={plan.planId}>
//               <strong>{plan.name}</strong> – ${plan.basePrice}
//             </li>
//           ))}
//         </ul>
//       )}
...
//  console.log(`${API_BASE_URL}/api/source/generate-token`,"The daata is showing here")
//  console.log(`${API_BASE_URL}/api/payment/initiate`,"The data is here")
// -------------------------------------
import React from "react";
      ...
      >
        ...
        <a
          ...
          className="flex items-center gap-4 p-4 rounded-xl
                     ...
                     transition-all duration-300 cursor-pointer"
        >
          <Mail className="text-blue-600 animate-bounce" />
          <span className="text-gray-700 font-medium">
...
//       navigate("/success", {
//         state: {
//           message: "Signup successful and our team will contact you in less than 24 hrs.",
...
//           </svg>
//         );
//      case "Delegate with Suit A Below":
//   return (
//     <>
//       <svg
//         xmlns="http://www.w3.org/2000/svg"
...
//     const [participantPhone, setParticipantPhone] = useState("");
//     const [participantOrganization, setParticipantOrganization] = useState("");
// // ===== FIXED: Consolidated useEffect for data fetching =====
// useEffect(() => {
//   let isMounted = true;
...
//     // Priority 1: Fetch by slug from URL path
//     if (slug) {
//       const token = sessionStorage.getItem("sourceToken") || localStorage.getItem("sourceToken");
//       if (!token) {
//         if (isMounted) setError("Access token missing. Please select a conference from the list.");
...
//       try {
//         const response = await axios.get(
//           `${API_BASE_URL}/api/conference/${slug}`,
...
//       }
//     } else {
//       // Priority 2: If no slug, check for token in query params (backward compatibility)
//       const queryParams = new URLSearchParams(location.search);
//       const sourceToken = queryParams.get("sourceToken");
...
//             // Navigate to slug-based URL for clean, shareable link
//             const conferenceSlug = response.data.conferenceName
//               .toLowerCase()
//               .replace(/[^a-z0-9]+/g, "-");
//             navigate(`/registration/${conferenceSlug}`, { replace: true });
//           } else {
//             setError("Invalid token. Please select a conference.");
//             setConferenceDetails(null);
//             setSelectedConferenceOption(null);
//           }
//         } catch (err) {
...
//         }
//       } else {
//         // No slug and no token: show selection interface
//         if (isMounted) setIsLoading(false);
//       }
//     }
//   };
...
//   //           setSelectedConferenceOption(preselectedOption || null);
//   //         } else {
//   //           // 🔹 Instead of showing "expired token", just clear and allow selection
//   //           setError("Invalid token. Please select a conference.");
//   //           setConferenceDetails(null);
//   //           setSelectedConferenceOption(null);
//   //         }
...
// //         //------------- adding the slugs-------------------------
// // const conferenceSlug = selectedConf.name
// //   .toLowerCase()
// //   .replace(/[^a-z0-9]+/g, "-");

// //         if (response.data.token) {
// //           localStorage.setItem("sourceToken", response.data.token); // ✅ Save token locally
// //           navigate(`/registration/${conferenceSlug}`);
// //           // navigate(`/registration?sourceToken=${response.data.token}`);
...
//     if (response.data.token) {
//       // FIXED: Store token in both storage locations
//       sessionStorage.setItem("sourceToken", response.data.token);
//       localStorage.setItem("sourceToken", response.data.token);
      
//       // Navigate using slug-based URL
//       const conferenceSlug = selectedConf.name
//         .toLowerCase()
//         .replace(/[^a-z0-9]+/g, "-");
//       navigate(`/registration/${conferenceSlug}`);
...
//             <div className="w-full md:w-2/3 bg-white p-7 md:p-9 rounded-2xl shadow-2xl text-center border border-gray-200 flex flex-col justify-between">
//               <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-6 leading-tight">
//                 Select a Conference
//               </h2>
//               <Select
...
//                 isClearable
//                 isSearchable
//                 className="mb-6 text-gray-800 text-base w-3/4 mx-auto"
//                 classNamePrefix="react-select"
//                 styles={{
//                   control: (base) => ({
//                     ...base,
...
//       {/* <h4 className="text-2xl font-bold text-blue-800 mb-6 text-center lg:text-center">
//         Conferences
//       </h4> */}

//       {/* ORIGINAL AMBER CARDS — UNCHANGED */}
//   <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
//     {allConferenceLists.hybridConfsAll.map((conf) => (
//       <div
//         key={conf.code}
//         className="flex flex-col justify-between bg-blue-50 rounded-2xl shadow-lg border border-blue-200 p-6 
//                   transition-all duration-300 hover:shadow-xl hover:-translate-y-2 h-full w-full"
//       >
//         {/* Icon */}
//         <div className="w-14 h-14 flex items-center justify-center rounded-full bg-blue-200 text-blue-700 mb-4 shadow-md">
//           <Mic2 className="h-7 w-7" />
//         </div>

//         {/* Title + Date */}
//         <div className="flex flex-col items-center text-center flex-1 overflow-hidden px-2">
//           <h5 className="font-extrabold text-xl text-blue-900 mb-2 leading-tight line-clamp-2">
//             {conf.title}
//           </h5>
...
//             onClick={() =>
//               handleProceedWithConference(
//                 allConferences.find((option) => option.value === conf.code)
//               )
//             }
...
// // Helper function to get category icons (retained as is)
// const getCatego ryIcon = (category) => {
//   switch (category) {
//     case "e-Poster Presentation":
//       return (
//         <svg
