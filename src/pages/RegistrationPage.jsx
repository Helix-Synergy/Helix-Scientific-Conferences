...
const RegistrationPage = () => {
    ...
    const conferences2025 = useMemo(() => {
        const currentYear = 2025; // As per explicit request
        const hybridConfs2025 = hybridsData.filter(conf => conf.year === currentYear);
        const webinarConfs2025 = webinarsData.filter(conf => conf.year === currentYear);
        return { hybridConfs2025, webinarConfs2025 };
    }, []);
    ...
    const allConferences = useMemo(() => {
        ...
        return [...webinarConfs, ...hybridConfs].map(conf => ({
            value: conf.id,
            label: `${conf.name} (${conf.type.charAt(0).toUpperCase() + conf.type.slice(1)})${conf.date ? ` - ${conf.date}` : ''}`,
            originalConf: conf
        }));
    }, []);
            ...
            <div className="container mx-auto px-4 sm:px-6 max-w-6xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-12 md:space-y-16">
                            ...
                            {registrationCategories.map(category => {
                                ...
                                return (
                                    <div key={category} className={`relative ${colors.cardBg} ${colors.cardShadow} ${colors.cardBorder} rounded-2xl flex flex-col transition-all duration-300 hover:shadow-lg hover:-translate-y-1`}>
                                        ...
                                        {/* Translucent Header Overlay */}
                                        <div className={`relative ${colors.overlayBg} rounded-t-2xl pt-10 pb-4 text-center z-10`}>
                                            <h3 className="text-xl sm:text-2xl font-bold text-gray-800 mt-6">{category}</h3>
                                        </div>

                                        {/* Main Card Content */}
                                        <div className="flex flex-col flex-grow p-4 pt-0 text-center">
                                            <p className="text-4xl font-extrabold text-gray-900 mb-4">${currentPrice.toFixed(2)}</p>
                                            ...
                                            <div className="flex items-center justify-center space-x-3 mt-auto">
                                                ...
                                                <span className="text-2xl font-semibold text-gray-900 w-10 text-center">{quantity}</span>
                                                <button
                                                    type="button"
                                                    onClick={() => handleQuantityChange(category, 1)}
                                                    className={`w-10 h-10 flex items-center justify-center rounded-full ${colors.buttonBg} text-white text-xl font-bold transition-all duration-200 ${colors.buttonHover}`}
                    ...
                    <div className="bg-white p-6 md:p-8 rounded-2xl shadow-md border border-gray-200">
                        <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-6 text-center">Your Contact Information</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label htmlFor="fullName" className="block text-base font-medium text-gray-700 mb-2">Full Name <span className="text-red-500">*</span></label>
                                <input
                                    type="text"
                                    id="fullName"
                                    className="mt-1 block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-base"
                            ...
                            <div>
                                <label htmlFor="email" className="block text-base font-medium text-gray-700 mb-2">Email Address <span className="text-red-500">*</span></label>
                                <input
                                    type="email"
                                    id="email"
                                    className="mt-1 block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-base"
                            ...
                            <div>
                                <label htmlFor="country" className="block text-base font-medium text-gray-700 mb-2">Country <span className="text-red-500">*</span></label>
                                <input
                                    type="text"
                                    id="country"
                                    className="mt-1 block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-base"
                            ...
                            <div className="md:col-span-2">
                                <label htmlFor="organization" className="block text-base font-medium text-gray-700 mb-2">Organization / University</label>
                                <input
                                    type="text"
                                    id="organization"
                                    className="mt-1 block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-base"
                    ...
                    <div className="bg-green-100 p-6 md:p-8 rounded-2xl shadow-lg text-green-900 flex flex-col items-center justify-center text-center border border-green-300">
                        <p className="text-lg md:text-xl font-semibold mb-3">Total Amount:</p>
                        <p className="text-4xl md:text-5xl font-extrabold mb-6">${totalAmount.toFixed(2)}</p>
                        <button
                            type="submit"
                            className="w-full max-w-sm bg-green-600 text-white px-7 py-3.5 rounded-xl text-lg font-bold
                                hover:bg-green-700 transition duration-300 transform hover:scale-105 shadow-md
